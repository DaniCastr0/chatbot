...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        auto newNode = std::find_if(_nodesraw.begin(), _nodesraw.end(), [&id](GraphNode *node) { return node->GetID() == id; });

                        // create new element if ID does not yet exist
                        if (newNode == _nodesraw.end())
                        {
                            _nodes.emplace_back(std::make_unique<GraphNode>(id));
                            _nodesraw.emplace_back(new GraphNode(id));
                            newNode = _nodesraw.end()- 1; // get iterator to last element
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            auto parentNode = std::find_if(_nodesraw.begin(), _nodesraw.end(), [&parentToken](GraphNode *node) { return node->GetID() == std::stoi(parentToken->second); });
                            auto childNode = std::find_if(_nodesraw.begin(), _nodesraw.end(), [&childToken](GraphNode *node) { return node->GetID() == std::stoi(childToken->second); });
    ...
    for (auto it = std::begin(_nodesraw); it != std::end(_nodesraw); ++it)
    {
